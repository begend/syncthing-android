...
- 正式支持 `HTTP` WebDAV server，而不只支持 `HTTPS`
- 在 `WebDAV` server 表单中，对 `http://` 地址显示显式风险提示
- 为 Android 14+ / targetSdk 36 的前台 `dataSync` 服务补齐权限要求

做出该决策的原因：

- 真实用户环境中存在只提供 `HTTP` 的 WebDAV 服务
- 如果应用仅允许 `HTTPS`，这类真实场景将无法使用

安全权衡：

- `HTTP` 会以明文方式传输用户名、密码和文件内容
- 该模式仅适合可信局域网、内网或 VPN 场景
- 面向公网或不可信网络时应优先使用 `HTTPS`

当前落地策略：

- 允许应用对 `HTTP` WebDAV server 建立连接
- 在 UI 中明确提醒使用风险，而不是静默放行

后续建议：

- 对 `http://` server 增加首次保存确认
- 在帮助文档中补充 `HTTP` 与 `HTTPS` 的使用建议
- 视发布策略决定是否区分 `debug` / `release` 的明文流量策略
...
- 新增“打开 App 时自动同步”开关
- 应用进入前台时自动触发一次 `WebDAV Sync All`
- 增加 15 分钟冷却时间，避免频繁切前台时重复触发

当前策略的边界：

- 这是一种“前台补跑”自动同步，不是完整后台调度
- 目前仍未覆盖定时同步、开机恢复、网络变化即时触发等完整场景
- folder 级约束（如 `Wi-Fi only` / `charging only` / `battery not low`）仍然生效

### 3.7 手动同步交互优化

当前已补上的手动同步体验优化：

- 主界面抽屉新增一级 `WebDAV Sync` 入口
- 非专家模式下默认隐藏“设备 / 状态”标签页，减少无关入口干扰
- 每个 WebDAV folder 的动作顺序调整为“先立即同步，再查看上次结果”
- 关键 WebDAV 文案补充中文

folder 级过滤能力：

- 支持按扩展名白名单过滤，如 `.txt,.md,.pdf`
- 支持按最大文件大小过滤，超过阈值的文件不会进入本次同步规划

当前价值：

- 手动同步入口更浅，用户更容易找到
- 文石这类设备上不再默认暴露太多 Syncthing 高级标签
- 用户可以主动缩小同步范围，减少误同步和大文件影响
